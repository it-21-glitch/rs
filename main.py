...
def index():
    ...
    return render_template('index.html', title=title, data_list_all=data_list_all, data_total=data_total,
...
def add_records():
    ...
    cursor.close()
    db_conn.close()
    return render_template('add_records.html', data_list_all=data_list_all, user_data_list_all=user_data_list_all)


# 获取工序,设备
@app.route("/get_flow_data", methods=["GET"])
def get_flow_data():
    """
...
# 附件获取
@app.route('/get_download_file_path', methods=["POST"])
...
# 附件下载
@app.route('/download_file_path', methods=["GET"])
...
def login():
    ...
    if user_name:
        return redirect(url_for('rs_index'))
    if form.validate_on_submit():  # csrf 验证
        ...
        try:
            ...
            return render_template('login.html', form=form, title=title, error="没有这个样的用户或登录信息错误！")
        cursor.fetchone()
...
# RS管理人员首页
@app.route("/rs/user/index", methods=["GET"])
def rs_index():
    per_page = 12  # 每页显示12条记录
    page = int(request.args.get('page', 1))
    form = VerifyForm()
...
# RS审核
@app.route("/rs/user/examine", methods=["POST"])
def examine():
    pk = request.form.get('pk')
    if not pk:
        return {'code': 500}
...
# RS查看考勤记录
@app.route("/rs/user/attendance", methods=["GET"])
def attendance():
    pk = request.args.get('pk')
    if not pk:
...
# RS退出登录
@app.route("/rs/user/logout", methods=["DELETE"])
def logout():
    try:
